...
static void maybe_load_kana_in_vram(void)
{
	...
	const Gfx *g_kana = gfx_get(GFX_SYS_KANA_FONT);
	s_pause.kana_vram_pos = gfx_load(g_kana, obj_vram_alloc(g_kana->size));
}

#define PAUSE_VRAM_TILE (PAUSE_VRAM_POSITION / 32)
...
static void plot_string(const char *str, int16_t x, int16_t y, int16_t pal)
{
	...
	while (*str)
	{
		...
		else
		{
			...
			if (a >= 'a' && a <= 'z')
			{
				md_vdp_write(VDP_ATTR(PAUSE_VRAM_TILE + 0x80 - 0x20 + (a & ~(0x20)), 0, 0, pal, 0));
			}
			else if (a == ' ')
			{
				md_vdp_write(VDP_ATTR(PAUSE_VRAM_TILE + 0x30, 0, 0, pal, 0));
			}
			else
			{
				md_vdp_write(VDP_ATTR(PAUSE_VRAM_TILE + 0x80 - 0x20 + (a), 0, 0, pal, 0));
			}
			plane_base += 2;
		}
	}
...
static inline void clear_window_plane(void)
{
	...
	const uint16_t fill_tile = VDP_ATTR(PAUSE_VRAM_TILE + 0x30, 0, 0, MAP_PAL_LINE, 0);
...
static inline void plot_map_side_borders(void)
{
	...
	{
		// Left side.
		md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE, 0, 0, MAP_PAL_LINE, 0));
		md_vdp_poke(plane_base - 2,
		         VDP_ATTR(PAUSE_VRAM_TILE + 0x31, 0, 0, MAP_PAL_LINE, 0));
		// Right side.
		md_vdp_poke(plane_base + right_offset,
		         VDP_ATTR(PAUSE_VRAM_TILE, 0, 0, MAP_PAL_LINE, 0));
...
static inline void plot_map_top_bottom_borders(void)
{
	...
	// Corner.
	md_vdp_poke(plane_base - 2, VDP_ATTR(PAUSE_VRAM_TILE + 0x32,
	                                  0, 0, MAP_PAL_LINE, 0));

	for (int16_t x = 0; x < PROGRESS_MAP_W + 2; x++)
	{
		// Top side.
		md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + 0x33,
		                              0, 0, MAP_PAL_LINE, 0));
		// Top padding.
		md_vdp_poke(plane_base + padding_offset, VDP_ATTR(PAUSE_VRAM_TILE,
		                                               0, 0, MAP_PAL_LINE, 0));
		// Bottom side.
...
static inline void plot_map(void)
{
	...
	uint16_t plane_base = md_vdp_get_plane_base(VDP_PLANE_WINDOW);
	const uint16_t tile_base = VDP_ATTR(PAUSE_VRAM_TILE, 0, 0, MAP_PAL_LINE, 0);
...
static inline void plot_map_cube_sector_extension(void)
{
	...
	{
		// Left side border
		md_vdp_poke(plane_base - 2, VDP_ATTR(PAUSE_VRAM_TILE + 0x31, 0, 0, MAP_PAL_LINE, 0));
		// Cells on bottom
...
static inline void plot_item_display_border(uint16_t x, uint16_t y)
{
	...
	md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + 0x85, 1, 0, ENEMY_PAL_LINE, 0));
	md_vdp_poke(plane_base + 2, VDP_ATTR(PAUSE_VRAM_TILE + 0x85, 0, 0, ENEMY_PAL_LINE, 0));
	plane_base += GAME_PLANE_W_CELLS * 2;
	md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + 0x85, 1, 1, ENEMY_PAL_LINE, 0));
	md_vdp_poke(plane_base + 2, VDP_ATTR(PAUSE_VRAM_TILE + 0x85, 0, 1, ENEMY_PAL_LINE, 0));
}
...
static void draw_cp_orbs_corner(void)
{
	...
	md_spr_put(x, y, VDP_ATTR(PAUSE_VRAM_TILE + 0x3C, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(2, 2));
	md_spr_put(x + 16, y, VDP_ATTR(PAUSE_VRAM_TILE + 0x40 + (2 * (cp_orbs / 10)), 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(1, 2));
	md_spr_put(x + 23, y, VDP_ATTR(PAUSE_VRAM_TILE + 0x40 + (2 * (cp_orbs % 10)), 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(1, 2));
}
...
static void draw_item_icons(void)
{
	...
	{
		md_spr_put(100, 196, VDP_ATTR(PAUSE_VRAM_TILE + 0x70, 0, 0, LYLE_PAL_LINE, 0), SPR_SIZE(1, 1));
	}
	
	if (progress->abilities & ABILITY_JUMP)
	{
		md_spr_put(124, 196, VDP_ATTR(PAUSE_VRAM_TILE + 0x72, 0, 0, LYLE_PAL_LINE, 0), SPR_SIZE(1, 1));
	}

	if (progress->abilities & ABILITY_PHANTOM)
	{
		md_spr_put(152, 192, VDP_ATTR(PAUSE_VRAM_TILE + 0x77, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(2, 2));
		...
		md_spr_put(152, 208, VDP_ATTR(PAUSE_VRAM_TILE + 0x54, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(1, 1));
		md_spr_put(160, 208, VDP_ATTR(PAUSE_VRAM_TILE + 0x55 + phantom_level, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(1, 1));
	}

	if (progress->abilities & ABILITY_KICK)
	{
		md_spr_put(184, 192, VDP_ATTR(PAUSE_VRAM_TILE + 0x73, 0, 0, LYLE_PAL_LINE, 0), SPR_SIZE(2, 2));
	}

	if (progress->abilities & ABILITY_ORANGE)
	{
		md_spr_put(212, 196, VDP_ATTR(PAUSE_VRAM_TILE + 0x71, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(1, 1));
	}
}
...
static inline void draw_map_pause_text(void)
{
	...
	md_spr_put(draw_x, draw_y,
	        SPR_ATTR(PAUSE_VRAM_TILE + 0x60, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(4, 2));
	md_spr_put(draw_x + 32, draw_y,
	        SPR_ATTR(PAUSE_VRAM_TILE + 0x68, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(4, 2));
}
...
static inline void draw_cube_sector_text(void)
{
	...
	const int16_t tile_base = SPR_ATTR(PAUSE_VRAM_TILE + 0x34, 0, 0, MAP_PAL_LINE, 0);
...
static inline void draw_map_location()
{
	...
	if (map_get_world_y_tile() <= PROGRESS_MAP_H && (progress->abilities & ABILITY_MAP))
	{
		...
		md_spr_put(draw_x, draw_y,
		        SPR_ATTR(PAUSE_VRAM_TILE + 0x2F, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(1, 1));
	}
	else
	{
		...
		md_spr_put(draw_x, draw_y,
		        SPR_ATTR(PAUSE_VRAM_TILE + 0x5A, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(2, 1));
		md_spr_put(draw_x + 27, draw_y,
		        SPR_ATTR(PAUSE_VRAM_TILE + 0x5C, 0, 0, MAP_PAL_LINE, 0), SPR_SIZE(4, 1));
	}
}
                              ...
                              const CharMapping *mapping, int16_t size)
{
	for (int16_t i = 0; i < size; i++)
	{
		md_spr_put(base_x + mapping[i].x, base_y + mapping[i].y,
		        SPR_ATTR(PAUSE_VRAM_TILE + 0x80 - 0x20 + mapping[i].val, 0, 0,
...
static void draw_you_got(PauseScreen screen)
{
	...
	switch (screen)
	{
		case PAUSE_SCREEN_LYLE_WEAK:
			...
			md_spr_put(88, base_y + 12, SPR_ATTR(CUBE_VRAM_POSITION/32, 0, 0, BG_PAL_LINE, 0), SPR_SIZE(2, 2));
			md_spr_put(216, base_y + 12, SPR_ATTR(CUBE_VRAM_POSITION/32, 0, 0, BG_PAL_LINE, 0), SPR_SIZE(2, 2));
		...
		case PAUSE_SCREEN_CP_ORB_15:
			...
			md_spr_put(88, base_y + 12, VDP_ATTR(PAUSE_VRAM_TILE + 0x3C, 0, 0, pal, 0), SPR_SIZE(2, 2));
			md_spr_put(216, base_y + 12, VDP_ATTR(PAUSE_VRAM_TILE + 0x3C, 0, 0, pal, 0), SPR_SIZE(2, 2));
	...
	// "YOU GOT:" header.
	md_spr_put(base_x + 13, base_y + 9, SPR_ATTR(PAUSE_VRAM_TILE + 0x7B, 0, 0,
	                       ENEMY_PAL_LINE, 0), SPR_SIZE(3, 1));
	md_spr_put(base_x + 40, base_y + 9, SPR_ATTR(PAUSE_VRAM_TILE + 0x7E, 0, 0,
	                       ENEMY_PAL_LINE, 0), SPR_SIZE(1, 1));
	md_spr_put(base_x + 48, base_y + 9, SPR_ATTR(PAUSE_VRAM_TILE + 0x7C, 0, 0,
	                       ENEMY_PAL_LINE, 0), SPR_SIZE(1, 1));
	md_spr_put(base_x + 56, base_y + 9, SPR_ATTR(PAUSE_VRAM_TILE + 0x7F, 0, 0,
	                       ENEMY_PAL_LINE, 0), SPR_SIZE(1, 1));
	md_spr_put(base_x + 64, base_y + 9, SPR_ATTR(PAUSE_VRAM_TILE + 0x8F, 0, 0,
	                       ENEMY_PAL_LINE, 0), SPR_SIZE(1, 1));
...
static void plot_get_sides(uint16_t plane_base)
{
	...
	{
		md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + 0x80, 0, 0, ENEMY_PAL_LINE, 0));
		md_vdp_poke(plane_base + 2*22, VDP_ATTR(PAUSE_VRAM_TILE + 0x80, 0, 0, ENEMY_PAL_LINE, 0));
		plane_base += GAME_PLANE_W_CELLS * 2;
	}
	// Corner piece.
	md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + 0x84, 0, 0, ENEMY_PAL_LINE, 0));
}
...
static void plot_get_top_and_bottom(uint16_t plane_base)
{
	...
	{
		md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + 0x83, 0, 0, ENEMY_PAL_LINE, 0));
		md_vdp_poke(plane_base + (GAME_PLANE_W_CELLS * 5 * 2), VDP_ATTR(PAUSE_VRAM_TILE + 0x83, 0, 0, ENEMY_PAL_LINE, 0));
		md_vdp_poke(plane_base + (GAME_PLANE_W_CELLS * 22 * 2), VDP_ATTR(PAUSE_VRAM_TILE + 0x83, 0, 0, ENEMY_PAL_LINE, 0));
		plane_base += 2;
	}
	// Corner.
	md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + 0x85, 0, 0, ENEMY_PAL_LINE, 0));
	md_vdp_poke(plane_base + (GAME_PLANE_W_CELLS * 5 * 2), VDP_ATTR(PAUSE_VRAM_TILE + 0x85, 0, 0, ENEMY_PAL_LINE, 0));
	md_vdp_poke(plane_base + (GAME_PLANE_W_CELLS * 22 * 2), VDP_ATTR(PAUSE_VRAM_TILE + 0x83, 0, 0, ENEMY_PAL_LINE, 0));
}
...
static void plot_get_side_grid(uint16_t plane_base)
{
	...
	for (uint16_t i = 0; i < ARRAYSIZE(grid_mapping) / 6; i++)
	{
		...
		{
			md_vdp_poke(plane_base, VDP_ATTR(PAUSE_VRAM_TILE + grid_mapping[map_index],
...
static void draw_debug_main_cursor()
{
	...
	md_spr_put((kdebug_left + 2) * 8, (kdebug_top + 2 + (s_pause.debug.main_cursor * 2)) * 8,
	        VDP_ATTR(PAUSE_VRAM_TILE + 0x70, 0, 0, LYLE_PAL_LINE, 0), SPR_SIZE(1, 1));
}
...
static inline void plot_hex16(uint16_t data, int16_t x, int16_t y, int16_t pal)
{
	uint16_t plane_addr = md_vdp_get_plane_base(VDP_PLANE_WINDOW) +
	                      2 * ((y * GAME_PLANE_W_CELLS) + x);
	const uint16_t tile_base = VDP_ATTR(PAUSE_VRAM_TILE + 0x60, 0, 0, pal, 0);
	md_vdp_set_autoinc(2);
...
static void draw_progress_edit_cursor()
{
	...
	if (s_pause.debug.progress_cursor_bit == 127)
	{
		md_spr_put((kdebug_left + 32) * 8, (kdebug_top + 2 + (s_pause.debug.progress_cursor_main)) * 8,
		        VDP_ATTR(PAUSE_VRAM_TILE + 0x87, 0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(4, 1));
	}
	else if (s_pause.debug.progress_cursor_bit >= 0)
	{
		md_spr_put((kdebug_left + 19 + (16 - s_pause.debug.progress_cursor_bit)) * 8,
		        ...
		        VDP_ATTR(PAUSE_VRAM_TILE + 0x87, 0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(1, 1));
	}
}
...
static void draw_press_button(int16_t x, int16_t y, int16_t frame)
{
	...
	const uint16_t base_attr = VDP_ATTR(PAUSE_VRAM_TILE, 0, 0, ENEMY_PAL_LINE, 0);
...
static void draw_pause_menu()
{
	...
	switch (s_pause.pause_choice)
	{
		case 0:
		case 1:
			md_spr_put(continue_x, continue_y, VDP_ATTR(PAUSE_VRAM_TILE + continue_offs,
			                                         0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(4, 1));
			md_spr_put(continue_x + 32, continue_y, VDP_ATTR(PAUSE_VRAM_TILE + continue_offs + 4,
			                                              0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(4, 1));
			md_spr_put(quit_x, quit_y, VDP_ATTR(PAUSE_VRAM_TILE + quit_offs,
			                                         0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(4, 1));
			md_spr_put(quit_x + 32, quit_y, VDP_ATTR(PAUSE_VRAM_TILE + quit_offs + 4,
			                                              0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(4, 1));
			md_spr_put(quit_x + 64, quit_y, VDP_ATTR(PAUSE_VRAM_TILE + quit_offs + 8,
			                                              0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(1, 1));
			break;
		case 2:
		case 3:
			md_spr_put(sure_x, sure_y, VDP_ATTR(PAUSE_VRAM_TILE + 0xC0, 0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(4, 1));
			md_spr_put(sure_x + 32, sure_y, VDP_ATTR(PAUSE_VRAM_TILE + 0xC0 + 4, 0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(3, 1));
			md_spr_put(yes_x, yes_y, VDP_ATTR(PAUSE_VRAM_TILE + yes_offs,
			                               0, 0, ENEMY_PAL_LINE, 0), SPR_SIZE(3, 1));
			md_spr_put(no_x, no_y, VDP_ATTR(PAUSE_VRAM_TILE + no_offs,
